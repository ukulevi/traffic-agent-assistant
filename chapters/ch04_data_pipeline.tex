...
\begin{tikzpicture}[scale=0.85]
  ...
  \draw[<->, thick, bkblue!80] (-0.4,0) -- (-0.4,\dy)
    node[midway, left, font=\small\bfseries, align=center] {Time Steps\\$T=12$\\(60 phút)};
  ...
  % Annotation arrows
  \node[font=\tiny, text=codegray, text width=2.5cm, align=left]
    at (\dx+\ox+0.8, \dy/2+\oy/2)
    {$B = 32$ hoặc $64$\\tùy VRAM GPU};
  \node[font=\tiny, text=codegray, text width=5cm, align=center]
    at (\dx/2, -0.7)
    {14 features: 3 GT + 5 KT + 3 KK + 3 PT};
  \node[font=\tiny, text=codegray, text width=2.2cm, align=right]
    at (-0.4-1.0, \dy/2)
