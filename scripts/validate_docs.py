...
def validate_public_artifacts(errors: list[str], contract: dict) -> None:
    ...
    checked_files += list((ROOT / "sections").glob("*.html"))
    checked_files += [
        p for p in (ROOT / "chapters").glob("*.tex")
    ...
    chapter_corpus = "\n".join(
        path.read_text(encoding="utf-8") for path in (ROOT / "chapters").glob("*.tex")
    )
    slide_corpus = "\n".join(
        path.read_text(encoding="utf-8") for path in (ROOT / "sections").glob("*.html")
    )
    ...
    feature_text = (ROOT / "chapters" / "ch04_data_pipeline.tex").read_text(
        encoding="utf-8"
    ).replace("\\_", "_")
